$(document).ready(function(){
    ...
    // #pcnav에 마우스를 올리면
        ...
        // #back그냥 slideDown으로 나타나게
    // #pcnav에서 마우스를 치우면
        // #pcnav(this)의 높이가 80px 이 되도록 애니메이트
        // #back그냥 slideUp으로 나타나게
    ...
    $(".mmain").click(function(){
        ...
        $(this).siblings(".mmain").children(".msub").delay(400).slideUp();
    });
    ...
    // 예) 세번째 대메뉴를 보고있는 상황이라면 => 홍보메뉴의 서브메뉴
    ...
    //#depth1_cont, #depth_cont에서 마우스를 치웠을때
        // 그 자신이 사라진다.
    $("#depth1_cont, #depth2_cont").mouseleave(function(){
        $(this).hide();
    });
    
    //페이지 내 모든 table태그에게 다음과 같이 따로따로 이야기 하겠다.
        // 그테이블 안쪽에 있는 내용을 잘 갈무리 해 두고
        // 그테이블 아래에 <div class="newtable"></div>를 만들고
        // 아까 갈무리한 내용을 그테이블 다음에 있는 .table의 안쪽에 넣어준다.
        // 그테이블을 지운다.
    ...
    // 여러 .tabbtn중에 하나를 클릭했을때
        ...
        // .tabbtn중에 nth번째에게는 "tactive"라는 클래스를 준다.
        // └-----------------┘this
    ...
    // .li-num마다 따로따로 다음과 같이 이야기 합시다.
        // 만약 그것 안에 들어있는 글자가 두자리 이상인가?
            // 그렇다면 그것의 스타일중에 
            // "leter-spacing"이라는 스타일의 값을 "-0.17em"으로 바꿔준다.
